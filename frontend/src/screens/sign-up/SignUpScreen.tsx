...
const signUpSchema = z.object({
  name: z.string().min(1, 'Nome é obrigatório').transform((value) => value.trim()),
...
export default function SignUpScreen({ onNavigateToLogin }: SignUpScreenProps) {
  ...
  const [signup, { loading }] = useMutation<SignupResponse, SignupVariables>(SIGNUP_MUTATION)
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-10 flex-col gap-8">
      ...
      <div className="w-full max-w-md rounded-4xl bg-white border border-border-primary p-8 shadow-lg shadow-slate-200/50">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-semibold text-text-heading">Criar conta</h1>
          <p className="text-sm text-text-primary mb-2">Comece a controlar suas finanças ainda hoje</p>
        </div>

        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate>
          <div>
            <label className="mb-2 block text-sm font-medium text-text-heading">Nome completo</label>
            <Input
